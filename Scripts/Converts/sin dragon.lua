...
if not isfile("ScriptBase/sin dragon.mp3") then
	writefile("ScriptBase/sin dragon.mp3", game:HttpGet("https://git.azula.wtf/CatThatDrinksSprite/moon-convert/raw/main/ScriptBase/sin%20dragon.mp3", true))
end
...
if game.Players.LocalPlayer.Character:FindFirstChild("Accessory (head)") then
	...
	for _, v in ipairs(game.Players.LocalPlayer.Character.Claw1:GetChildren()) do
		if v:IsA("BasePart") then
			v.Transparency = 1
		end
	end
	for _, v in ipairs(game.Players.LocalPlayer.Character.Claw2:GetChildren()) do
		if v:IsA("BasePart") then
			v.Transparency = 1
		end
	end
else
	sendNotification("Moon Convert", "It is recommended to wear the hats used for this script... You can get the hats by using \"get hats -h sin dragon\"", 7)
end
